...
public class SolutionPlain {
    ...
    private static void buildGraph(int m, Map<Integer, Node> nodeMap, List<Edge> edgeList) {
        for (int i = 0; i < m; i++) {
            ...
            Edge edge1 = new Edge(nodeMap.get(node2));
            Edge edge2 = new Edge(nodeMap.get(node1));
    ...
    private static boolean solve(Node start, Node dest, int expectedMinResult, int n, int k, int t) {
        ...
        firstMove(start, dest, n, expectedMinResult);
        for (int i = expectedMinResult; i < k; i++) {
            if (!move(start, dest, n, expectedMinResult, c)) {
                return false;
            }
        }
        return true;
    ...
    private static void firstMove(Node start, Node dest, int size, int expectedMinResult) {
        Map<Node, Node> pred = new HashMap<>();
        findShortestPath(start, dest, size, expectedMinResult, pred);
        Node current = dest;
        while (current != start) {
            Node prev = pred.get(current);
            Edge edge = prev.edgeMap.get(current);
            edge.brokenness = expectedMinResult;
            current.edgeMap.get(prev).brokenness = expectedMinResult;
            current = prev;
        }
    }

    private static boolean move(Node start, Node dest, int size, int expectedMinResult, Container t) {
        Map<Node, Node> pred = new HashMap<>();
        int totalMinBrokenness = findShortestPath(start, dest, size, expectedMinResult, pred);
        Node current = dest;
        while (current != start) {
            Node prev = pred.get(current);
            Edge edge = prev.edgeMap.get(current);
            if (edge.brokenness > expectedMinResult) {
                edge.brokenness = expectedMinResult;
                current.edgeMap.get(prev).brokenness = expectedMinResult;
            }
            edge.brokenness++;
            current.edgeMap.get(prev).brokenness++;
            current = prev;
        }

        t.t -= (totalMinBrokenness);
        if (t.t < 0) return false;

        return true;
    }

    private static int findShortestPath(Node nodeStart, Node nodeDest, int size, int expectedMinResult, Map<Node, Node> pred) {
        ...
        while (start != null) {
            for (Edge edge : start.node.edgeList) {
                ...
                if (nextMinDistance < distances[edge.right.id]) {
                    pred.put(edge.right, start.node);

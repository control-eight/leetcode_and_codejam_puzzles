...
public class Solution2 {
    ...
    private static int findShortestPath(Node nodeStart, Node nodeDest, int size, int expectedMinResult, Edge[] pred) {
        Queue<Key>[] heap = new Queue[size];

        Key start = new Key(nodeStart, 0);
        ...
        while (start != null) {
            for (Edge edge : start.node.edgeList) {
                ...
                if (nextMinDistance < distances[edge.right.id]) {
                    ...
                    if (heap[nextMinDistance] == null) {
                        heap[nextMinDistance] = new LinkedList<>();
                    }
                    heap[nextMinDistance].add(new Key(edge.right, nextMinDistance));
                    max = Math.max(edge.brokenness, max);
                }
            }
            while (cursor < heap.length && (heap[cursor] == null || heap[cursor].isEmpty())) {
                cursor++;
            }
            if (cursor < heap.length) {
                start = heap[cursor].poll();

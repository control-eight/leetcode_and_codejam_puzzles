...
/**
 * Created by alex.bykovsky on 9/6/17.
 */
public class BestTimeBuySellStockCooldown {

	public static void main(String[] args) {
		assertEquals(12, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 4, 7, 2, 11}));
		assertEquals(3, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 2, 3, 0, 2}));
		assertEquals(3, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 2, 4}));
		assertEquals(5, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 2, 3, 4, 5, 6}));
		assertEquals(6, new BestTimeBuySellStockCooldown().maxProfit(new int[] {6, 1, 3, 2, 4, 7}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {2, 4, 1, 7, 11}));
		assertEquals(9, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 3, 2, 4, 7, 8, 10}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {6, 8, 2, 6, 3, 7, 1, 7, 6, 0}));
		assertEquals(7, new BestTimeBuySellStockCooldown().maxProfit(new int[] {5, 5, 5, 6, 8, 1, 6, 1, 5, 1}));
		assertEquals(6, new BestTimeBuySellStockCooldown().maxProfit(new int[] {7, 7, 4, 4, 3, 4, 8, 0, 5, 4}));
		assertEquals(6, new BestTimeBuySellStockCooldown().maxProfit(new int[] {7, 7, 4, 4, 3, 4, 8, 0, 5, 4}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {2, 1, 0, 4, 8, 9, 5, 7, 6, 6}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {9, 9, 6, 6, 0, 7, 1, 5, 5, 8}));
		assertEquals(12, new BestTimeBuySellStockCooldown().maxProfit(new int[] {6, 7, 0, 6, 7, 2, 4, 2, 2, 6}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {7, 3, 8, 0, 6, 2, 7, 3, 5, 0}));
		assertEquals(13, new BestTimeBuySellStockCooldown().maxProfit(new int[] {3, 8, 10, 0, 6, 1, 7}));
		assertEquals(15, new BestTimeBuySellStockCooldown().maxProfit(new int[] {2, 7, 0, 9, 8, 5, 9, 9, 2, 4}));
		assertEquals(8, new BestTimeBuySellStockCooldown().maxProfit(new int[] {7, 3, 4, 8, 9, 4, 6, 3, 5, 0}));
		assertEquals(12, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 5, 5, 4, 9, 0, 3, 6, 1, 3}));
		assertEquals(15, new BestTimeBuySellStockCooldown().maxProfit(new int[] {1, 8, 1, 2, 2, 4, 2, 8, 0, 6}));
		assertEquals(10, new BestTimeBuySellStockCooldown().maxProfit(new int[] {5, 2, 3, 0, 3, 5, 6, 8, 1, 5}));
		assertEquals(188, new BestTimeBuySellStockCooldown().maxProfit(new int[] {70, 4, 83, 56, 94, 72, 78, 43, 2, 86, 65, 100}));
		assertEquals(9, new BestTimeBuySellStockCooldown().maxProfit(new int[] {7, 1, 8, 3, 5, 3, 4, 4, 5, 2}));
		assertEquals(16, new BestTimeBuySellStockCooldown().maxProfit(new int[] {0, 6, 3, 2, 7, 2, 5, 3, 8, 2}));
		assertEquals(70, new BestTimeBuySellStockCooldown().maxProfit(new int[] {3, 8, 10, 0, 6, 15, 1, 7, 20, 0, 50}));
	...
	public int maxProfit(int[] prices) {
		...
		return Math.max(maxProfitRested(prices, 1), -prices[0] + maxProfitBought(prices, 1));
	}

	private Map<Container, Integer> cache = new HashMap<>();

	private int maxProfitRested(int[] prices, int i) {
		Container key = new Container(State.RESTED, i);
		Integer result = cache.get(key);
		if(result != null) return result;

		if(i >= prices.length) return 0;

		result = Math.max(-prices[i] + maxProfitBought(prices, i + 1), maxProfitRested(prices, i + 1));
		cache.putIfAbsent(key, result);
		return result;
	}

	private int maxProfitBought(int[] prices, int i) {
		Container key = new Container(State.BOUGHT, i);
		Integer result = cache.get(key);
		if(result != null) return result;

		if(i >= prices.length) return 0;

		result = Math.max(prices[i] + maxProfitSold(prices, i + 1), maxProfitBought(prices, i + 1));
		cache.putIfAbsent(key, result);
		return result;
	}

	private int maxProfitSold(int[] prices, int i) {
		Container key = new Container(State.SOLD, i);
		Integer result = cache.get(key);
		if(result != null) return result;

		if(i >= prices.length) return 0;

		result = maxProfitRested(prices, i + 1);
		cache.putIfAbsent(key, result);
		return result;
	}
	...
	private static class Container {
		...
		public Container(State state, int i) {
			this.state = state;
			this.i = i;
		}

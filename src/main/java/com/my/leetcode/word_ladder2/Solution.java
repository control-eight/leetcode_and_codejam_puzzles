package com.my.leetcode.word_ladder2;
...
public class Solution {
	...
	private static long hashCodeAcc;

	public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {

		Chars endWordChars = new Chars(endWord.toCharArray());

		Set<Chars> wordSet = new HashSet<>();

		for (String s : wordList) {
			wordSet.add(new Chars(s.toCharArray()));
		}

		if(!wordSet.contains(endWordChars)) return Collections.emptyList();

		Node startNode = new Node(new Chars(beginWord.toCharArray()));

		Queue<Node> nodeQueue = new LinkedList<>();
		nodeQueue.offer(startNode);

		List<Map<Chars, Node>> solutions = new ArrayList<>(50);

		for(int i = 0; i < 50; i++) {
			solutions.add(new HashMap<>());
		}
		solutions.get(0).put(startNode.word, startNode);

		Map<Chars, Set<Chars>> possible = new HashMap<>();

		//int minPathLength = Integer.MAX_VALUE;
		int minPathLength = ladderLength(startNode.word, endWordChars, wordList, wordSet);

		int count = 0;

		int max1 = minPathLength / 2;
		int max2 = minPathLength - minPathLength / 2;
		...
		while (!nodeQueue.isEmpty()) {
			...
			//if(node.pathLength > max1) break;

			if(node.pathLength + 1 > max1) {
				continue;
			}

			if(possible.containsKey(node.word)) {
				for (Chars newWord : possible.get(node.word)) {
					minPathLength = getMinPathLength(endWordChars, wordSet, nodeQueue, minPathLength, solutions, possible, node,
							newWord, true, max1, max2);
				}
			} else {

				for(int i = 0; i < node.word.length(); i++) {
					Chars wordChanger = new Chars(node.word);
					for (int c = 97; c <= 122; c++) {

						wordChanger.changeAt(i, (char) c);

						if(wordChanger.equals(node.word)) continue;

						minPathLength = getMinPathLength(endWordChars, wordSet, nodeQueue, minPathLength, solutions, possible, node, wordChanger,
								false, max1, max2);
					}
				}
			}
		...
		Node endNode = new Node(endWordChars);
		endNode.forward = false;
		endNode.pathLength = minPathLength - 1;

		nodeQueue = new LinkedList<>();
		nodeQueue.offer(endNode);

		solutions.get(endNode.pathLength).put(endNode.word, endNode);
		...
		for (Node node : solutions.get(max1 - 1).values()) {

			if(node.forward) {

				List<List<String>> tmpResult1 = new ArrayList<>();
				List<List<String>> tmpResult2 = new ArrayList<>();

				for (Edge edge : node.edgeMap.values()) {
					if(edge.right != null && edge.right.forward) {
						List<String> path = new ArrayList<>();
						path.add(new String(node.word.value));

						generatePaths(edge.left, path, tmpResult1, true);

					} else if(edge.left == null) {
						for (Edge edgeRight : edge.right.edgeMap.values()) {

							List<String> path = new ArrayList<>();
							path.add(new String(edge.right.word.value));

							generatePaths(edgeRight.right, path, tmpResult2, false);
						}
					}
				}

				for (List<String> leftPart : tmpResult1) {
					for (List<String> rightPart : tmpResult2) {
						List<String> leftPartNew = new ArrayList<>(leftPart);
						leftPartNew.addAll(rightPart);
						result.add(leftPartNew);
					}
				}
			}
		}
		...
		if(minPathLength == Integer.MAX_VALUE) return Collections.emptyList();

		return result;
	}

	private void generatePaths(Node node, List<String> path, List<List<String>> result, boolean left) {
		...
		if(left) {
			path.add(0, new String(node.word.value));
		} else {
			path.add(new String(node.word.value));
		}
		...
		for (Edge edge : node.edgeMap.values()) {
			...
			if(left) {
				generatePaths(edge.left, newPath, result, left);
			} else {
				generatePaths(edge.right, newPath, result, left);
			}
		}
	}

	private int getMinPathLength(Chars endWord, Set<Chars> wordSet, Queue<Node> nodeQueue, int minPathLength, List<Map<Chars, Node>> solutions, Map<Chars, Set<Chars>> possible, Node node, Chars newWord, boolean exist, int max1, int max2) {
		if (wordSet.contains(newWord)) {

			long start = System.nanoTime();

			if(!exist) {
				possible.putIfAbsent(node.word, new HashSet<>());
				acc += System.nanoTime() - start;
				possible.get(node.word).add(new Chars(newWord));
			}

			Map<Chars, Node> currentLevel = solutions.get(node.pathLength + (node.forward? 1: -1));

			Node existingNode = currentLevel.get(newWord);

			if (existingNode != null) {

				if(!existingNode.edgeMap.containsKey(node.word)) {

					Edge edge = new Edge();

					if(node.forward) {
						edge.left = solutions.get(node.pathLength).get(node.word);
					} else {
						edge.right = solutions.get(node.pathLength).get(node.word);
					}

					existingNode.edgeMap.put(node.word, edge);
				}

			} else {
				boolean equals = endWord.equals(newWord);
				boolean b = node.forward? node.pathLength + 1 < max1: node.pathLength + 1 > max2;
				if(b || equals) {

					newWord = new Chars(newWord);
					...
					newNode.edgeMap = new HashMap<>();
					newNode.edgeMap.put(node.word, e);

					if(node.forward) {
						newNode.pathLength = node.pathLength + 1;
					} else {
						newNode.pathLength = node.pathLength - 1;
					}

					if (equals) {
						minPathLength = Math.min(minPathLength, newNode.pathLength);
					}

					currentLevel.put(newWord, newNode);
	...
	private int ladderLength(Chars beginWord, Chars endWord, List<String> wordList, Set<Chars> wordSet) {

		Set<Chars> visited = new HashSet<>();
		...
		while (!nodeQueue.isEmpty()) {
			...
			for(int i = 0; i < node.word.length(); i++) {
				...
				for (int c = 97; c <= 122; c++) {
					...
					Chars newWord = new Chars(wordChanger);
		...
		return minPathLength == Integer.MAX_VALUE? 0: minPathLength + 1;
	}
	...
	private static class Node {
		...
		private Map<Chars, Edge> edgeMap;

		private boolean forward = true;

		public Node(Chars word) {
			this.word = word;
		}
	}
	...
	private static class Chars {
		...
		private void toShortArr(char[] value) {
			this.sValue = new byte[value.length];
			for (int i = 0; i < value.length; i++) {
				sValue[i] = (byte) value[i];
			}
		}

		public Chars(Chars word) {
			this.value = Arrays.copyOf(word.value, word.length());
			toShortArr(value);
		}

		public void changeAt(int index, char c) {
			value[index] = c;
			sValue[index] = (byte) c;
		}
		...
		public int hashCode() {
			...
			hashCodeAcc += System.nanoTime() - start;
			return h;
		}
		...
		public boolean equals(Object anObject) {
			...
			if (anObject instanceof Chars) {
				Chars anotherChars = (Chars)anObject;
				int n = value.length;
				if (n == anotherChars.value.length) {
					char v1[] = value;
					char v2[] = anotherChars.value;
					int i = 0;
					while (n-- != 0) {
						if (v1[i] != v2[i])
							return false;

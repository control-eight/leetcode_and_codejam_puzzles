...
public class LFUCache {
	...
	public LFUCache(int capacity) {
		...
		this.countNodeMap = new HashMap<>(capacity);
	}
	...
	private void evict() {
		...
		Node someNode = countNodeMap.get(root.count);
		if(someNode == root) {
			countNodeMap.remove(someNode.count);
		}
	...
	private void moveNodeRight(Node node) {
		if(node.right != null) {
			if(node.count >= node.right.count) {
				...
				updateCountNodeMap(node, node.left);
			}
		} else {
			updateCountNodeMap(node, node.left);
		}
	...
	private void moveRight(Node node) {
		...
		Node right = countNodeMap.get(node.count);
		if(right == null) {
			right = countNodeMap.get(node.right.count);
		}
		...
		updateCountNodeMap(node, tmpLeft);
	}

	private void updateCountNodeMap(Node node, Node left) {
		if(countNodeMap.get(node.count - 1) == node) {
			if(left != null && node.count - 1 == left.count) {
				countNodeMap.put(left.count, left);
			}
		}
		...
		countNodeMap.put(node.count, node);
	}
	...
	private static void test6() {
		...
		for (String part : input.split("\\]")) {
			...
			if(arr.length == 1) {
				System.out.println(cache.get(Integer.parseInt(arr[0])));;
			} else {
	...
	private static void test3() {
		...
		String input = "[1,1],[2,2],[3,3],[4,4],[4],[3],[2],[1],[5,5],[1],[2],[3],[4],[5]";

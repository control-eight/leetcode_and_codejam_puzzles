...
public class Solution4 {
    ...
    public static void main(String[] args) {
        System.out.println(new Solution4().catMouseGame(new int[][] {
    ...
    private int c(int[][] graph) {
        ...
        boolean[][] visitedStates = new boolean[graph.length][2];
        visitedStates[1][0] = true;
        visitedStates[2][1] = true;
    ...
    public int play(int[][]graph, boolean mouseMove, int mousePos, int catPos, boolean[][] visitedStates, int[][][] cache) {
        ...
        int res = mouseMove? 2: 0;
        int pos = mouseMove? mousePos: catPos;
        for (int node : graph[pos]) {
            if (mouseMove || node != 0) {
                if (!mouseMove || !visitedStates[node][0]) {
                    if (mouseMove) {
                        visitedStates[node][0] = true;
                    }
                    int l = play(graph, !mouseMove, mouseMove ? node : mousePos, mouseMove ? catPos : node, visitedStates, cache);
                    res = mouseMove? Math.min(res,l): Math.max(res, l);
                    if (mouseMove) {
                        visitedStates[node][0] = false;
                    }
                }
            }
        }
